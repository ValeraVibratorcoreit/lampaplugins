(function() {
    ...
    console.log('YouTube Plugin: Starting initialization process.');
    ...
    function YouTubeComponent(object) {
        ...
        this.create = function() {
            console.log('YouTube Plugin: YouTubeComponent.create() called.');
            // Удаляем создание search_input, так как Lampa.Filter предоставит свой
            filter.onSearch = function(value) {
                console.log('YouTube Plugin: Filter search triggered with query:', value);
                component_self.search(value);
            };

            filter.onBack = function() {
                console.log('YouTube Plugin: Filter back action.');
        ...
        this.render = function() {
            console.log('YouTube Plugin: YouTubeComponent.render() called.', root_element);
            return root_element;
        };

        this.start = function() {
            console.log('YouTube Plugin: YouTubeComponent.start() called.');
        ...
        this.loading = function(status) {
            console.log('YouTube Plugin: Loading status:', status);
        ...
        this.reset = function() {
            console.log('YouTube Plugin: Resetting results.');
            scroll.clear();
        ...
        this.empty = function(msg) {
            console.log('YouTube Plugin: Displaying empty message:', msg);
        ...
        this.search = function(query) {
            console.log('YouTube Plugin: Performing search for query:', query);
            if (!query) {
            ...
            network.fetch(api_url, function(data) {
                console.log('YouTube Plugin: Search API response:', data);
                ...
                console.error('YouTube Plugin: Search API error.');
                component_self.empty(Lampa.Lang.translate('search_error'));
            });
        };

        this.displayResults = function(items) {
            console.log('YouTube Plugin: Displaying search results:', items);
        ...
        this.playVideo = function(item) {
            console.log('YouTube Plugin: Playing video:', item.title);
            Lampa.Player.play({
                title: item.title,
                url: item.url,
                is_youtube: true
        ...
        this.pause = function() {
            console.log('YouTube Plugin: Video paused.');
        };
        this.stop = function() {
            console.log('YouTube Plugin: Video stopped.');
        };
        this.destroy = function() {
            console.log('YouTube Plugin: YouTubeComponent destroyed.');
            network.clear();
            files.destroy();
            scroll.destroy();
        };
    ...
    function startPlugin() {
        console.log('YouTube Plugin: startPlugin() called.');
        var manifest = {
            type: 'youtube',
            version: '0.0.1',
            name: 'YouTube',
        ...
        function resetTemplates() {
            console.log('YouTube Plugin: resetTemplates() called.');
            Lampa.Template.add('lampac_content_loading', "<div class="online-empty">
                ...
                <div class="online-empty__templates">
                    ...
                    <div class="online-empty-template"></div>
                    <div class="online-empty-template"></div>
                </div>
            </div>");
        ...
        function addYoutubeMenuItem() {
            console.log('YouTube Plugin: addYoutubeMenuItem() called.');
        ...
        Lampa.Listener.follow('app', function(e) {
            if (e.type == 'ready') {
                console.log('YouTube Plugin: App ready event triggered.');
    ...
    if (!window.youtube_plugin_init) {
        ...
        startPlugin();
        // Эти вызовы также нужны, если плагин загружается раньше события app:ready
        resetTemplates();
        $('body').append(Lampa.Template.get('lampac_css', {}, true));
    }
})();
